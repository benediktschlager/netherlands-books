    ...
    return (
        <div className="flex items-stretch justify-center pt-3 h-screen">
            <div className="flex flex-col gap-3 w-80 h-96 min-h-96 ">
                {editing ? <input type="text" value={name}
                                  onChange={({currentTarget}) => setName(currentTarget.value)}></input> :
                    <h1>{name}</h1>}
                <h2>About me</h2>
                {editing ? (<textarea className={textAreaClasses + " w-[100%] h-[100%]"}
                                      onChange={({currentTarget}) => setAbout(currentTarget.value)} value={about}/>) : (
                    <p className={textAreaClasses}>{about}</p>)}
                {status === 'inactive' &&
                    <div className="flex gap-1 justify-center items-center  min-w-min w-min border rounded bg-sky-400">
                        <PauseIcon/>
                        <div className="bg-sky-800">Inactive</div>
                    </div>}
                {status === 'active' && <div
                    className="flex gap-1 justify-center  min-w-min w-min items-center rounded border bg-green-300">
                    <ReaderIcon/>
                    <div className="bg-green-400">Active</div>
                </div>}
                ...
                <footer className="mt-auto w-100%">
                    {showEditButton &&
                        <Button className="bg-blue-500 w-24 h-24 text-lg p-1 rounded b-1 border float-right"
                                onClick={() => setEditing(!editing)}><SymbolIcon/> Edit</Button>}
                </footer>
            </div>
        </div>
    )
